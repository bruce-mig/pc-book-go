	...
	for {
		...
		if err != nil {
			return logError(status.Errorf(
				codes.Unknown,
				"cannot receive stream request: %v", err,
			))
		}
		...
		if err != nil {
			return logError(status.Errorf(
				codes.Internal,
				"cannot find laptop: %v", err,
			))
		}
		if found == nil {
			return logError(status.Errorf(
				codes.NotFound,
				"laptopID %s is not found", laptopID,
			))
		}
		...
		if err != nil {
			return logError(status.Errorf(
				codes.Internal,
				"cannot add rating to the store: %v", err,
			))
		}
		...
		if err != nil {
			return logError(status.Errorf(
				codes.Unknown,
				"cannot send stream response: %v", err,
			))
		}
	}
	return nil
